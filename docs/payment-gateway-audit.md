...
Fixes cover stale reservation cleanup, persisted client-scoped creation idempotency, authenticated image documentation, a two-minute reconciliation window, strict transaction timestamps and QRIS settlement filtering, full-body transport timeouts, poll recovery and pagination, durable leased webhook retries, input limits, merchant QR readiness, and the real GoBiz analytics field name `transaction_status` (without this mapping, every live settlement was ignored). A concurrent login-lockout counter defect discovered in native MongoDB tests was also corrected in both adapters.
